...
def configure_ngrok():
    ngrok_token = "2lsfGgRGJDviHQ9ViCkLxsNgGtz_7Za6jtt1PixmUwdwoQgXR"
    try:
...
def update_soul_txt(public_url):
    with open("vipmodsxadmin1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in vipmodsxadmin1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("vipmodsxadmin1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated vipmodsxadmin1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update vipmodsxadmin1.txt on VPS: {str(e)}")
